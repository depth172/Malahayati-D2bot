...
def getBanshee(isTest=False):
	...
	for i in range(3):
		vendor = []
		vendor.append(requests.get("https://www.bungie.net/Platform/Destiny2/3/Profile/4611686018482984497/Character/2305843009576344512/Vendors/672118013/?components=401,402,310,305", headers=headers).json())
		vendor.append(requests.get("https://www.bungie.net/Platform/Destiny2/3/Profile/4611686018482984497/Character/2305843009576344511/Vendors/672118013/?components=401,402,310,305", headers=headers).json())
		vendor.append(requests.get("https://www.bungie.net/Platform/Destiny2/3/Profile/4611686018482984497/Character/2305843009405135001/Vendors/672118013/?components=401,402,310,305", headers=headers).json())
	...
	# 販売アイテムリストを生成
	# 0 = ハンター, 1 = タイタン, 2 = ウォーロック
	salesListRaw = vendor[0]['Response']['categories']['data']['categories'][3]['itemIndexes'] + vendor[1]['Response']['categories']['data']['categories'][3]['itemIndexes'] + vendor[2]['Response']['categories']['data']['categories'][3]['itemIndexes']
	salesList = list(set(salesListRaw))
	...
	for index, w in enumerate(salesList):
		# 改ページ時の処理
		if formFeed:
			# フォントと背景画像の準備
			resImg = io.BytesIO()
		...
		for c in range(3):
			if str(w) not in vendor[c]['Response']['sales']['data']:
				continue
			
			# データ取得
			lWeaponHash = vendor[c]['Response']['sales']['data'][str(w)]['itemHash']
			...
			# パスから画像・ウォーターマークを取得
			try:
				lWeaponMWHash = vendor[c]['Response']['itemComponents']['sockets']['data'][str(w)]['sockets'][7]['plugHash']
			...
			for i in range(1, 6):
				try:
					perkQuantity = len(vendor[c]['Response']['itemComponents']['reusablePlugs']['data'][str(w)]['plugs'][str(i)])
				...
				for r in range(perkQuantity):
					...
					try:
						if perkQuantity == 1:
							lWeaponPerkHash = vendor[c]['Response']['itemComponents']['sockets']['data'][str(w)]['sockets'][s]['plugHash']
						else:
							lWeaponPerkHash = vendor[c]['Response']['itemComponents']['reusablePlugs']['data'][str(w)]['plugs'][str(s)][r]['plugItemHash']
			...
			if index + 1 == m:
				perkCount += perkMax
				imgHeight = 225 + 245 * math.ceil((index + 1 - (p - 1) * 4) / 2) + 90 * perkCount
